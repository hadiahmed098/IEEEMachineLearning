...
def processScreen(np_array):
    ...
    processed_screen[processed_screen != 255] = 0  # Sets all non white pixels to 0
    processed_screen[processed_screen == 255] = 1  # Sets all white pixels to 1

    return processed_screen

def getAction(proc_input, weights_one, weights_two):
    # Compute hidden layer
    hidden = relu(weights_one.dot(proc_input))
    # Compute output layer
    output = relu(weights_two.dot(hidden))
